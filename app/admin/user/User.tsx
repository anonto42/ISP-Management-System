...
const User = ({user}:{user: undefined | usertype }) => {
    if(user === undefined){
        redirect("/admin")
    }
  ...
  return (
    <div>
        ...
        { deposite && <DiposideUserBalance setPosition={setDeposite} user={user} /> }
        ...
        <section className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-w-[1600px] w-full mx-auto gap-4 overflow-y-auto'>
            ...
            <div className='shadow-md bg-[#f7f7f7] rounded-md overflow-clip'>
                ...
                <div className='user_info_box'>
                    ...
                    <div className='text-green-500 border px-3 py-2 text-sm rounded-full'>{user.expireDate== null? "UNDEFIEND" : user.expireDate.toDateString()}</div>
                </div>
                <div className='user_info_box'>
